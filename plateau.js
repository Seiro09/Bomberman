...
class Pion {
  ...
  * si une image est déjà présente dans la case de destination, alors il y a collision
  */
  move(x,y){
    if(x != this.position.x || y != this.position.y) {
      ...
      td = this.getCase();
      if(td.lastChild != null) td.removeChild(td.lastChild);
...
function setCaseSelectionnable(td,pion){
  ...
  var color = (td.firstChild == undefined)? 'green' : 'red';
  
  td.onmouseenter = function(e){ //la case devient coloré différement si elle est survolée
  ...
  td.style.color = color; //ajoute un contour pour indiquer que la case peut être choisit
